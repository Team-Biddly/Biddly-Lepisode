...
export class PreStandardController {
  ...
  @Get('file-info')
  @ApiOperation({ summary: '파일 URL 정보 조회 (파일명 추출)' })
  @ApiOkResponse({ type: FileInfoResponseDto })
  async getFileInfo(@Query('url') url: string): Promise<FileInfoResponseDto> {
    if (!url) throw new BadRequestException('URL is required');

    const cacheKey = `file-info:${url}`;
    const cached = await this.cache.get<string>(cacheKey);
    if (cached) return { filename: cached };

    const filename = await this.preStandardService.getFileFilename(url);
    await this.cache.set(cacheKey, filename, 0);
    return { filename };
  }
