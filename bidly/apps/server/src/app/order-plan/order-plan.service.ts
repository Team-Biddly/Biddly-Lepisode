...
export class OrderPlanService {
  ...
  async createExcelFile(ids: string[]): Promise<Buffer> {
      ...
      .then((orderPlans) =>
        orderPlans.map((orderPlan, i) => {
          ...
          return {
            ...
            연락처: 전화번호 || '',
            게시일시: orderPlan.게시일시
              ? new Date(orderPlan.게시일시)
                  .toISOString()
                  .slice(0, 16)
                  .replace('T', ' ')
              : undefined,
            발주금차도급금액:
              typeof orderPlan.발주금차도급금액 === 'bigint'
                ? String(orderPlan.발주금차도급금액)
                : orderPlan.발주금차도급금액?.toString(),
          };
        }),
      );
    ...
    orderPlans.forEach((orderPlan) => {
      Object.keys(orderPlan).forEach((key) => {
        const value = orderPlan[key];
        const valueLength =
